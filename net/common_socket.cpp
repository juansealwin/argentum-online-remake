...
Socket Socket::accept() {
  ...
  if (fd == -1) throw std::invalid_argument("closed socket!");
  Socket skt;
  skt.fd = fd;
  return std::move(skt);
}
...
int Socket::send(const void *msg, const size_t length) const {
  ...
  do {
    bytes = ::send(this->fd, &buffer[total_bytes_sent], remaining_bytes,
                   MSG_NOSIGNAL);
...
int Socket::recv(void *response, const size_t length) const {
  ...
  do {
    bytes = ::recv(this->fd, &buffer[total_bytes_received], remaining_bytes, 0);
