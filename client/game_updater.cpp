...
void GameUpdater::run() {
  try {
    while (is_running) {
      ...
      blocking_map.updateMap(next_status);
      ...
      for (it = next_status.begin(); it != next_status.end(); it++) {
        ...
        if (it2 != current_status.end()) {
          ...
          if (!(it2->second.isEqual(it->second))) {
            // Si cambio hacemos un update del personaje
            current_map->updateCharacter(it->first, it->second.getX(),
                                        it->second.getY());
          ...
          current_map->loadCharacter(renderer, it->second.getTypeCharacter(),
                                    it->first, it->second.getX(),
                                    it->second.getY());
        }
      }
